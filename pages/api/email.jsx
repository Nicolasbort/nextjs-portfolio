...
export default (req, res) => {
    ...
    if (req.method == "POST")
    {
        ...
        transporter.sendMail(mailOptions, (err, data) => {
            if (err){
                res.status(200).json({ error: { pt: `Falha ao enviar o email. Erro: ${err.response}`, en: `Failed to send email. Error: ${err.response}` } })
            }else {
                res.status(200).json({ message: { pt: "E-mail enviado com sucesso!", en: "E-mail successfully sent!" } })
            }
        })
    }
    else
    {
        res.status(400).json({ error: {pt: "Método de request inválido", en: "Invalid request method"} });
    }
}